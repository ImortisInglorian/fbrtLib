...
private sub fb_hHookConScroll( handle as fb_ConHooks ptr, x1 as long, y1 as long, x2 as long, y2 as long, rows as long )
	...
	if ( iClearFrom <> iClearTo ) then
		...
		for iClearPos = iClearFrom to iClearTo - 1
			...
			FillConsoleOutputCharacter( hnd, 32, wid, coord, @written )
		next
	end if
...
private function fb_hHookConWrite ( handle as fb_ConHooks ptr, buffer as const any ptr, length as size_t ) as long
	...
	next
	result = WriteConsoleOutput( hnd, lpBuffer, dwBufferSize, dwBufferCoord, @srWriteRegion )
	return result
end function
...
sub fb_ConsolePrintBufferEx( buffer as const any ptr, _len as size_t, mask as long )
	...
	scope
		...
		fb_ConPrintTTY( @hooks, pachText, _len, TRUE )

...
#define FB_KEY_LIST_SIZE (ARRAY_LENGTH(fb_ext_key_entries))
...
#define FB_KEY_CODES_SIZE (ARRAY_LENGTH(fb_asc_key_codes))
	...
	if ( (dwControlKeyState and ENHANCED_KEY) <> 0 andalso ( is_ext_code <> 0 ) ) then
		...
		for i = 0 to FB_KEY_LIST_SIZE - 1
			...
			if (entry.scan_code = wVsCode) then
				dim as FB_KEY_CODES codes = entry.codes
		...
		if ( wVsCode < FB_KEY_CODES_SIZE ) then
			dim as FB_KEY_CODES codes = fb_asc_key_codes(wVsCode)

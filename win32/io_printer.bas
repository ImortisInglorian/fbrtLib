...
private function GetDefaultPrinterName( ) as ubyte ptr
	...
	if ( count = 0 ) then
		dim as HMODULE hMod = LoadLibrary(__TEXT(sadd("winspool.drv")))
		if (hMod <> NULL) then
			#ifdef UNICODE
			dim as LPCTSTR pszPrinterId = __TEXT(sadd("GetDefaultPrinterW"))
			#else
			dim as LPCTSTR pszPrinterId = __TEXT(sadd("GetDefaultPrinterA"))
			#endif
			dim as FnGetDefaultPrinter pfnGetDefaultPrinter = cast(FnGetDefaultPrinter, GetProcAddress(hMod, pszPrinterId))
...
private function fb_hPrinterBuildListDefault( list as FB_LIST ptr, iStartPort as long ) as long
	...
	if ( printer_name <> NULL ) then
		if ( fb_hListDevFindName( list, printer_name ) = NULL ) then
			...
			do
				iPort += 1
				sprintf( @Buffer(0), "LPT%d", iPort )
				node = fb_hListDevFindDevice( list, @Buffer(0) )
			loop while( node <> NULL )

			fb_hListDevElemAlloc ( list, @Buffer(0), printer_name )
		end if
		free( printer_name )
	end if
...
private function fb_hPrinterBuildListOther( list as FB_LIST ptr, iStartPort as long ) as long
	...
	for i = 0 to count - 1
		...
		if ( fb_hListDevFindName( list, printer->pPrinterName ) = NULL ) then
			...
			fb_hListDevElemAlloc ( list, @Buffer(0), printer->pPrinterName )
		end if
	next
	free(printers)
...
function fb_PrinterOpen( devInfo as DEV_LPT_INFO ptr, iPort as long, pszDevice as const ubyte ptr ) as long
	...
	if ( result <> FB_RTERROR_OK ) then
		if ( *lpt_proto->emu <> 0 ) then
			dim as long iJob
			dim as DOCINFO _docInfo
			memset( @_docInfo, 0, sizeof(_docinfo) )
			_docInfo.cbSize = sizeof(DOCINFO)
			_docInfo.lpszDocName = doc_title
			iJob = StartDoc( hDc, @_docInfo )
			if ( iJob <= 0 ) then
			...
			DocInfo.pDatatype = __TEXT(sadd("RAW"))
...
private sub EmuPrint_RAW( pInfo as W32_PRINTER_INFO ptr, pText as const any ptr, uiLength as size_t, isunicode as long )
	while( uiLength <> 0 )
		if ( isunicode = 0 ) then
			...
			EmuPageStart( pInfo )
			TextOut( pInfo->hDc, pInfo->Emu.dwCurrentX, pInfo->Emu.dwCurrentY, cast(LPCSTR, @ch), 1 )
		else
			dim as FB_WCHAR ch = *cast(FB_WCHAR ptr, pText)
			pText += sizeof(ch)

			EmuPageStart( pInfo )
			TextOutW( pInfo->hDc, pInfo->Emu.dwCurrentX, pInfo->Emu.dwCurrentY, cast(LPCWSTR, @ch), 1 )
		end if
...
private sub EmuPrint_TTY( pInfo as W32_PRINTER_INFO ptr, pText as const any ptr, uiLength as size_t, isunicode as long )
	...
	if ( isunicode = NULL ) then
		while( uiLength <> 0 )
			...
			for ui = 0 to uiLength - 1
				dim as long iFound = FALSE
				dim as ubyte ptr ch = @(cast(ubyte ptr, pText))[ui]
				select case ch
					case 12:
						/' FormFeed '/
						iFound = TRUE
		...
		while( uiLength <> 0 )
			...
			for ui = 0 to uiLength - 1
				dim as long iFound = FALSE
				dim as ubyte ptr ch = cast(ubyte ptr, @(cast(FB_WCHAR ptr, pText))[ui])
				select case ch
					case 12:
						/' FormFeed '/
						iFound = TRUE

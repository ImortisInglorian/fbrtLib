...
private sub fb_hHookConScroll ( handle as fb_ConHooks ptr, x1 as long, y1 as long, x2 as long, y2 as long, rows as long )
    dim as fb_PrintInfo ptr pInfo = cast(fb_PrintInfo, handle->Opaque)
...
private function fb_hHookConWrite( handle as fb_ConHooks ptr, buffer as any const ptr, size_t length as size_t ) as long
    dim as fb_PrintInfo ptr pInfo = cast(fb_PrintInfo ptr, handle->Opaque)
    dim as HANDLE hnd = pInfo->hOutput
    dim as FB_WCHAR const ptr pachText = cast(FB_WCHAR const ptr, buffer)
    ...
    for i = 0 length - 1
        dim as CHAR_INFO ptrpCharInfo = lpBuffer + i
...
sub fb_ConsolePrintBufferWstrEx( buffer as FB_WCHAR const ptr, chars as size_t, mask as long )
    dim as FB_WCHAR const ptr pachText = cast(FB_WCHAR const ptr, buffer)
    ...
    scope
        ...
        scope
            dim as COORD dwCoord = { cast(SHORT, hooks.Coord.X), cast(SHORT, hooks.Coord.Y) }
...
sub fb_ConsolePrintBufferWstr( buffer as FB_WCHAR const ptr, mask as long )

...
private function fb_hFileLineInputEx( handle as FB_FILE ptr, dst as any ptr, dst_len as ssize_t, fillrem as long ) as long
    ...
    select case ( mode )
		case eIM_Read:
			...
			_len = readlen = 0
			while (handle->hooks->pfnEof(handle) = 0)
				...
				if ( res = FB_RTERROR_OK and read_len = 1) then
					...
					if ( ch = 13 ) then
						...
						if ( res = FB_RTERROR_OK and ch <> 10 and read_len = 1) then
				...
				if ( do_add <> 0 or  handle->hooks->pfnEof( handle ) ) then
					...
					buffer(_len) = 0
					dim as FBSTRING ptr src = fb_StrAllocTempDescF( @buffer(0), _len + 1)
					if ( readlen = 0 ) then
				...
				if ( res <> FB_RTERROR_OK or do_break ) then
					exit while
				end if
			wend
			if ( readlen = 0 ) then
